module.exports = function(schema) {
  ...
  var getDocs = function(req, res, data) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      db.model('artisan').find({
        ...
        async.eachLimit(docs, 100, function(e, big_callback) {
            ...
            async.eachLimit(paiementsst.interventions, 100, function(inter, small_callback) {
  ...
  var getExcel = function(data) {
    var rtn = [];
    rtn.push(['Ajouté par', 'Date', 'id', 'Artisan ID', 'Artisan Nom Societé', 'Type', 'Mode', 'Numero Cheque',
      'Base', 'Final'])
    _.each(data, function(sst) {
      var ids = _.map(sst.list.__list, 'id')
      _.each(sst.list.__list, function(e) {
        rtn.push([e.login, moment(e.date).format('l hh:mm'), e.id, sst.id, sst.nomSociete, e.type, e.mode,
          e.numeroCheque, e.montant.base, e.montant.final])
      })
    })
    return rtn;
  }
