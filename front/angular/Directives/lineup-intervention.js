 ...
 angular.module('edison').directive('lineupDevis', function(tabContainer, FiltersFactory, ContextMenu, LxProgressService, edisonAPI, DataProvider, $routeParams, $location, $rootScope, $filter, config, ngTableParams) {
     ...
     return {
         ...
         controller: function($scope) {
             ...
             var currentFilter;
             var currentHash = $location.hash();
             var dataProvider = new DataProvider('devis', $routeParams.hashModel);
             ...
             var lastChange = 0;
             $rootScope.$on('DEVIS_CACHE_LIST_CHANGE', function(event, newData) {
                 if (tabContainer.getCurrentTab() && $scope.tab.fullUrl === tabContainer.getCurrentTab().fullUrl && newData._date > lastChange) {
                     dataProvider.applyFilter(currentFilter, $scope.tab.hash, $scope.customFilter);
                     $scope.tableParams.reload();
                     // $scope.tableParams.orderBy($scope.tableParams.$params.sorting)
             ...
             $scope.contextMenu = new ContextMenu('devis')
