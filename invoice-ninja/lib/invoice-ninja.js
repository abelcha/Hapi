...
function generateItemList(items, currencyFormat, template) {
  ...
  if (items) {
    items.forEach(function(element, i, array) {
      ...
      .replace(/OS/g, element.id)
      .replace(/DATE/g, element.date)
      .replace(/CAT/g, element.cat)
      .replace(/MONTANT/g, util.format(currencyFormat, element.montant))
      .replace(/COM/g, element.com)
      .replace(/TOTAL/g, util.format(currencyFormat, element.total));
    });
  }
